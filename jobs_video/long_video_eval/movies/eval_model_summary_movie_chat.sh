...
# use general summary
python eval_long_videos_movie_chat.py --fps=$fps --neighbours_global=$NEIGHBOURS  --start=$START --end=$END --batch_size $BATCH_SIZE  --ckpt $CKPT_PATH  --exp_name=$exp_name\
      --dataset_path $dataset_path --annotation_json_folder $annotation_json_folder --use_openai_embedding $use_openai_embedding
